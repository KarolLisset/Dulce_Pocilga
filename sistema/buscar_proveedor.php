...
<body class="listas">
	...
	<section id="container">
		...
		<h1 class="Titulos"><i class="fa-solid fa-table-list"></i> Lista de proveedores</h1>
      ...
      //Agarra el valor del buscador y lo aplica a cada campo hasta que haya una similitud//
			$sql_registe=mysqli_query($connect, "SELECT count(*)as total_registro FROM proveedor WHERE (codproveedor LIKE '%$busqueda%' OR proveedor LIKE '%$busqueda%' OR contacto LIKE '%$busqueda%' OR telefono LIKE '%$busqueda%')AND estatus=1");
			...
			//Se añadio el WHERE estatus=1 para que aparezcan solo los usuarios con estatus activo, si no se incluye el estatus en las tablas, no es necesario incluirlo
				$query=mysqli_query($connect, "SELECT * FROM proveedor
				 WHERE
				(codproveedor LIKE '%$busqueda%' OR
				 ...
				 AND
				 estatus=1 ORDER BY codproveedor ASC LIMIT $desde,$por_pagina"); //Ordena el ID de manera ascendente c://
