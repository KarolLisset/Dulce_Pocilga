...
<select name="proveedor" id="search_proveedor">
	...
	?>
			...
			$sql_registe=mysqli_query($connect, "SELECT count(*)as total_registro FROM producto as p WHERE $where");
			...
			//Se añadio el WHERE estatus=1 para que aparezcan solo los usuarios con estatus activo, si no se incluye el estatus en las tablas, no es necesario incluirlo
				$query=mysqli_query($connect, "SELECT p.codproducto, p.descripcion, p.precio, p.existencia, pr.proveedor, p.foto FROM producto p INNER JOIN proveedor pr ON p.proveedor=pr.codproveedor WHERE $where ORDER BY p.codproducto DESC LIMIT $desde,$por_pagina"); //Ordena el ID de manera ascendente c://
...
?>
				...
				<li><a href="?pagina=<?php echo $total_pagina; ?>&<?php echo $buscar;?>"><i class="fas fa-step-forward"></i></a></li>
			<?php 
		}
