...
<body class="listas">
	...
	<section id="container">
		...
		<table>
			...
			$sql_registe=mysqli_query($connect, "SELECT count(*)as total_registro FROM usuario WHERE estatus=1");
			...
			//Se añadio el WHERE estatus=1 para que aparezcan solo los usuarios con estatus activo, si no se incluye el estatus en las tablas, no es necesario incluirlo
				$query=mysqli_query($connect, "SELECT u.idusuario, u.nombre, u.correo, u.usuario, r.rol FROM usuario u INNER JOIN rol r ON u.rol= r.idrol WHERE estatus=1 ORDER BY u.idusuario ASC LIMIT $desde,$por_pagina"); //Ordena el ID de manera ascendente c://
